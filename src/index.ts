...
function extractMessageText(msg: any, ctx?: any): string {
  if (!msg && !ctx) return "";
  ...
  register(api: OpenClawPluginApi) {
    ...
    if (autoRecall) {
      api.on("before_agent_start", async (event: any, ctx: any) => {
        ...
        api.logger.info(
          `[openclaw-memory-tencentdb] [Hook:before_agent_start] Fired (prompt: "${prompt.slice(0, 50)}...", team: ${client.getTeamId()}, agent: ${client.getAgentId()})`,
        );
        ...
        try {
          ...
          try {
            api.logger.info(
              `[openclaw-memory-tencentdb] [Hook:before_agent_start] Querying conversation memory for "${prompt.slice(0, 40)}..."`,
            );
            ...
            api.logger.info(
              `[openclaw-memory-tencentdb] [Hook:before_agent_start] Conversation search returned ${validMessages.length} items.`,
            );
          ...
          api.logger.info(
            `[openclaw-memory-tencentdb] [Hook:before_agent_start] Successfully injected ${recalledSections.length} memory sections into context (${contextBlock.length} chars)`,
          );
    ...
    if (autoCapture) {
      api.on("agent_end", async (event: any, ctx: any) => {
        ...
        api.logger.info(
          `[openclaw-memory-tencentdb] [agent_end] Syncing turn to TencentDB in background (user: "${lastUserText.slice(0, 30)}...", assistant: "${lastAssistantText.slice(0, 30)}...")`,
        );
    ...
    api.registerTool({
      ...
      execute: async (rawArgs: any) => {
        try {
          ...
          api.logger.info(
            `[openclaw-memory-tencentdb] [Tool:tdai_conversation_search] Executing query: "${query}" (limit: ${limit})`,
          );
          if (!query) {
          ...
          api.logger.info(
            `[openclaw-memory-tencentdb] [Tool:tdai_conversation_search] Query "${query}" returned ${Array.isArray(msgs) ? msgs.length : 0} hits.`,
          );
