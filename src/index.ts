    ...
    if (autoCapture) {
      const handleCapture = async (event: any, ctx: any) => {
        ...
        if (!userText) {
          ...
          } else if (sessionPrompts.has(sessionId)) {
            userText = sessionPrompts.get(sessionId) || "";
          } else if (fallbackLastPrompt) {
        ...
        if (!userText || !assistantText) {
          api.logger.debug?.(
            `[openclaw-memory-tencentdb] Capture skipped: incomplete turn (user: ${Boolean(userText)}, assistant: ${Boolean(assistantText)})`,
          );
          return;
        }
        ...
        api.logger.info(
          `[openclaw-memory-tencentdb] Syncing turn to TencentDB Panel for session: ${sessionId} (user: ${userText.slice(0, 40)}...)`,
        );
        ...
        client
          ...
          .catch((err: any) => {
            api.logger.warn(
              `[openclaw-memory-tencentdb] Background turn sync failed: ${err.message || String(err)}`,
            );
          });
      };
