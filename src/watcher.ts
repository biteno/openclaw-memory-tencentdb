...
export class OpenClawSessionWatcher {
  ...
  private querySqlitePython(dbPath: string, sql: string, params: any[] = []): any[] {
    const pyCode = `
...
`;
  ...
  private inspectAndSyncSqlite(dbPath: string, isBaseline: boolean) {
    ...
    for (const t of tables) {
      ...
      if (!tableName || tableName.startsWith("sqlite_") || tableName.startsWith("_")) continue;
      ...
      const colNames = cols.map((c: any) => String(c.name || "").toLowerCase());
      ...
      // Query new rows
      try {
        const rows = this.querySqlitePython(
          dbPath,
          `SELECT rowid as _rowid, * FROM "${tableName}" WHERE rowid > ? ORDER BY rowid ASC`,
          [lastWatermark],
        );
