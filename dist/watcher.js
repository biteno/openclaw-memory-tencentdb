...
export class OpenClawSessionWatcher {
    ...
    fileOffsets = new Map();
    isRunning = false;
    constructor(client, logger, customWatchDirs) {
        this.client = client;
        this.logger = logger;
    ...
    processFile(filePath, isBaseline) {
        try {
    ...
    parseAndSyncMarkdown(content, fileName) {
    ...
    parseAndSyncJsonl(content, fileName) {
    ...
    syncTurn(user, assistant, sourceFile) {
        if (!user || !assistant)
            return;
        ...
        this.logger.info(`[openclaw-memory-tencentdb] [File-Watcher] Discovered new turn in ${sourceFile} — syncing to TencentDB (user: "${user.slice(0, 35)}...")`);
        this.client
            ...
            .then(() => {
            this.logger.info(`[openclaw-memory-tencentdb] [File-Watcher] Successfully synced turn from ${sourceFile} to TencentDB L0!`);
        })
            .catch((err) => {
            this.logger.warn(`[openclaw-memory-tencentdb] [File-Watcher] Sync failed for ${sourceFile}: ${err.message || String(err)}`);
        });
    }
}
