...
export class OpenClawSessionWatcher {
    ...
    sessionPendingUser = new Map();
    constructor(client, logger) {
        this.client = client;
        this.logger = logger;
    }
    start(pollIntervalMs = 3000) {
        if (this.isRunning)
            return;
        this.isRunning = true;
            ...
            if (fs.existsSync(agentsDir)) {
                try {
                    ...
                    for (const ag of agents) {
                        ...
                        if (fs.existsSync(agDb))
                            results.add(agDb);
                        const agDbDirect = path.join(agentsDir, ag.name, "openclaw-agent.sqlite");
                        if (fs.existsSync(agDbDirect))
                            results.add(agDbDirect);
                    }
                }
                catch { }
            }
    ...
    processTextFile(filePath, isBaseline) {
        try {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    syncTurn(user, assistant, sourceDesc) {
        if (!user || !assistant)
            return;
        ...
        this.logger.info(`[openclaw-memory-tencentdb] [Watcher] Discovered new turn via ${sourceDesc} — syncing to TencentDB (user: "${user.slice(0, 40)}...")`);
        this.client
            ...
            .then(() => {
            this.logger.info(`[openclaw-memory-tencentdb] [Watcher] Successfully synced turn from ${sourceDesc} to TencentDB L0!`);
        })
            .catch((err) => {
            this.logger.warn(`[openclaw-memory-tencentdb] [Watcher] Sync failed for ${sourceDesc}: ${err.message || String(err)}`);
        });
    }
}
