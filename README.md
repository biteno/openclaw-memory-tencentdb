...
> **Remote TencentDB Agent Memory Plugin for OpenClaw** — Bidirektionale Anbindung an zentrale TencentDB Memory-Cluster (`tencentdb.itsc.local`).
...
Dieses Plugin verbindet **OpenClaw** nativ mit einem zentralen **TencentDB Agent Memory Cluster** (`http://tencentdb.itsc.local`). 

Im Gegensatz zum offiziellen lokalen NPM-Paket (das eine isolierte SQLite-Datenbank auf der lokalen Maschine betreibt) und im Gegensatz zum transparenten Proxy (der individuelle API-Keys und Token-Budgets im LiteLLM-Proxy verliert), arbeitet dieses Plugin als **schlanker, nativer HTTP-Client**:
...
```jsonc
{
  "plugins": {
    "openclaw-memory-tencentdb": {
      "enabled": true,
      "coreUrl": "http://tencentdb.itsc.local:8420",
      "importUrl": "http://tencentdb.itsc.local:8125",
      "knowledgeUrl": "http://tencentdb.itsc.local:8424",
      "userKey": "sk-mem-YOUR_TENCENTDB_API_KEY",
      "teamId": "team-thpa5ncu0p",
...
> ⚠️ **Wichtig:** Verwende unter `agentId` immer die für dein OpenClaw-Profil vorgesehene Container-ID (z. B. aus der [Team-IDs & Agent-IDs Übersicht](https://wiki.biteno.com/books/agent-konfiguration/page/tencentdb-agent-memory-team-ids-agent-ids-ubersicht-docker13-tencentdbitsclocal)).

### Parameter-Referenz:

| Parameter | Typ | Standardwert | Beschreibung |
| :--- | :--- | :--- | :--- |
| `coreUrl` | String | `http://tencentdb.itsc.local:8420` | URL des TencentDB MemoryCore (:8420) für semantische L0–L3 Suche. |
| `importUrl` | String | `http://tencentdb.itsc.local:8125` | URL des MemoryPanel (:8125) für den asynchronen Turn-Import. |
| `knowledgeUrl` | String | `http://tencentdb.itsc.local:8424` | URL des MemoryKnowledge (:8424) für Wiki- und CodeGraph-Suche. |
| `userKey` | String | `sk-mem-...` | Persönlicher TencentDB Access Key aus `.admin-key`. |
| `teamId` | String | `team-thpa5ncu0p` | Team-Identifier / Mandant in TencentDB. |
| `agentId` | String | `agt-...` | Eindeutige Kennung dieser OpenClaw-Instanz. |
...
```text
[openclaw-memory-tencentdb] Loaded plugin (team: team-thpa5ncu0p, agent: agt-..., autoRecall: true, autoCapture: true)
```
